...
void test_game_save_load(void)
{
  game_state_t game_1;
  int ok = load_game("saves/game_1.txt", &game_1);
  tps_assert(ok);

  ok = save_game("saves/game_tmp_save.txt", &game_1);
  tps_assert(ok);

  game_state_t game;
  ok = load_game("saves/game_tmp_save.txt", &game);
  tps_assert(ok);
